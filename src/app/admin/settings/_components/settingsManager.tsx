            ...
            <CardContent>
              <div className='space-y-4'>
                {items.map((item) => {
                  ...
                  const currentUnit = units[item.key] ?? 'ms';

                  // ms 설정: 현재 단위로 변환된 표시 값 계산
                  const displayValue = isMs
                    ? (() => {
                        const raw = Number(values[item.key] ?? '0');
                        return isNaN(raw) ? (values[item.key] ?? '') : String(msToDisplay(raw, currentUnit));
                      })()
                    : (values[item.key] ?? '');
                    ...
                    >
                      ...
                      {isMs ? (
                        <div className='flex gap-2'>
                          <Input
                            ...
                            onChange={(e) => {
                              ...
                              if (e.target.value === '' || isNaN(num)) {
                                ...
                                handleChange(item.key, String(displayToMs(num, currentUnit)));
                              }
                            }}
                          ...
                          <Select
                            value={currentUnit}
                            onValueChange={(newUnit: TimeUnit) => {
                              setUnits((prev) => ({ ...prev, [item.key]: newUnit }));
                            }}
                            disabled={mutation.isPending}
                          >
                        ...
                        <div className='flex items-center gap-2'>
                          <Input
                            ...
                            className='flex-1'
                            value={(() => {
                              const mins = cronToMinutes(values[item.key] ?? '');
                            ...
                            onChange={(e) => {
                              ...
                              if (e.target.value === '' || isNaN(num) || num < 1) {
                                ...
                                handleChange(item.key, minutesToCron(num));
                              }
                            }}
                            disabled={mutation.isPending}
                          />
