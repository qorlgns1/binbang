...
const WORKER_REQUIRED_ENV_VARS = ['DATABASE_URL', 'KAKAO_CLIENT_ID', 'KAKAO_CLIENT_SECRET'];
/**
 * 웹 앱용 환경변수 검증
 */
 ...
 * 공통 검증 로직
 */
function validateEnvVars(keys, context) {
    const missing = keys.filter((key) => !process.env[key]);
    if (missing.length > 0) {
        throw new Error(`❌ ${context} 필수 환경변수가 설정되지 않았습니다:\n` +
