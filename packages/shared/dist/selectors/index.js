...
// ============================================
function getHardcodedFallback(platform) {
 ...
 */
export function buildExtractorCode(_platform, selectorCache) {
    const { selectors, patterns } = selectorCache;
    // 셀렉터가 없으면 빈 문자열 반환 (기존 하드코딩 로직 사용)
    const hasSelectors = selectors.price.length > 0 ||
 ...
 */
export function getPlatformSelectors(platform) {
 ...
 */
export async function loadPlatformSelectors(platform, force = false) {
    const cached = cache.get(platform);
    ...
    try {
        ...
        console.log(`[selectors] Loaded ${platform}: ${dbSelectors.length} selectors, ${dbPatterns.length} patterns` +
 ...
 */
export function invalidateSelectorCache(platform) {
    const invalidated = [];
    if (platform) {
 ...
 */
export async function preloadSelectorCache() {
    const platforms = ['AIRBNB', 'AGODA'];
