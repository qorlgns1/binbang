...
const DEFAULTS: Record<string, { env?: string; default: string }> = {
...
let cache: SystemSettingsCache | null = null;
let lastLoadedAt = 0;

// ── 값 해소: DB → env → default ──

function resolveValue(key: string, dbMap: Map<string, string>): string {
...
function buildCache(dbMap: Map<string, string>): SystemSettingsCache {
 ...
 */
export async function loadSettings(force = false): Promise<SystemSettingsCache> {
 ...
 */
export function getSettings(): SystemSettingsCache {
  if (!cache) {
