...
const AGODA_DATA_EXTRACTOR = `function() {
  ...
  try {
    ...
    for (const script of scripts) {
      ...
      const hotelIdMatch = text.match(/hotelId[:\t\n\r ]*([0-9]+)/);
      if (hotelIdMatch) {
        platformId = hotelIdMatch[1];
        break;
      }
      const propertyIdMatch = text.match(/propertyId[:\t\n\r ]*([0-9]+)/);
      if (propertyIdMatch) {
...
export async function checkAgoda(
  ...
  if (!options.selectorCache) {
    throw new Error('selectorCache is required for checkAgoda');
  }
