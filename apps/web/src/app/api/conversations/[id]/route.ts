...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    console.error('Failed to fetch conversation', { requestId, userId, error });
...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return jsonResponse({ success: true, conversationId, title: parsedBody.data.title, requestId });
  } catch (error) {
    console.error('Failed to update conversation title', { requestId, userId, error });
