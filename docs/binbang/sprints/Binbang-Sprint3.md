...
| 스냅샷 보존 기간 | 30일 (env: `BINBANG_SNAPSHOT_RETENTION_DAYS`) |
| 스톨 임계값 | 폴링 주기 × 2 |
| apps/binbang 퇴역 | **W5 이전에 완료** (Sprint 3 진행 중 처리) |
...
**env vars**:
- `BINBANG_VACANCY_COOLDOWN_HOURS=24` (기본값)
- `BINBANG_PRICE_DROP_COOLDOWN_HOURS=6` (기본값)
...
| 변수 | 기본값 | 설명 | 추가 시점 |
|---|---|---|---|
| `BINBANG_VACANCY_COOLDOWN_HOURS` | `24` | vacancy 쿨다운 시간 | W5-D2 |
| `BINBANG_PRICE_DROP_COOLDOWN_HOURS` | `6` | price_drop 쿨다운 시간 | W5-D2 |
| `BINBANG_SNAPSHOT_RETENTION_DAYS` | `30` | 스냅샷 보존 일수 | W5-D5 |
| `BINBANG_PRICE_DROP_THRESHOLD` (전역) | `0.1` | 전역 가격 하락 임계값 (숙소별 `priceDropThreshold` 미설정 시) | W6-D2 |
