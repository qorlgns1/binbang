...
- 웹, 트래블, 워커, 공유 코드, 데이터베이스의 소유권을 패키지 단위로 분리
...
- 여행 AI 플래너 애플리케이션
- App Router, Server Component, Route Handler 중심
- DB 접근은 `apps/travel/src/services/**` 경유 원칙 준수
...
- Next.js 앱(`apps/web`, `apps/travel`)은 `@workspace/worker-shared/*` import 금지

---

## 6. 마이그레이션 진행 현황 (2026-02-28 기준)
...
- web/travel/worker/공유/DB가 독립적으로 변경 및 배포 가능하다.
