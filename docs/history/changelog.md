...
## v2.26.0 – 운영 케이스 액션로그/제출 관리 고도화

운영자가 케이스를 추적하고 처리하는 관리 기능을 확장하고, 공개 페이지의 다국어 콘텐츠를 보강했습니다.

- **P0-8 케이스 운영 강화**: 운영자 액션 로그, 분쟁 응답 템플릿 도입
- **관리자 제출 워크플로우 확장**: `/admin/submissions`, `/admin/intake-mappings` 추가 및 제출→케이스 생성 연계
- **다국어 콘텐츠 확장**: FAQ/About 페이지 5개국어 지원, 랜딩 네비게이션 정리
- **안정성 개선**: `price_quote` 유효성 검사/복사 방지 등 PR #61 리뷰 피드백 반영

## v2.25.0 – robots 정책 개선

검색엔진 크롤링 정책을 명확하게 유지할 수 있도록 robots 생성 로직을 정리했습니다.

- **robots.txt 생성 개선**: 배포 환경별 크롤링 허용/차단 규칙 표현을 명확화

## v2.24.0 – i18n 코어 도입 및 5개국어 확장

공개/앱 라우트 전반에 국제화 코어를 적용하고, 다국어 SEO 체계를 고도화했습니다.

- **i18n 코어 적용**: `next-intl` 기반 라우팅/미들웨어/로케일 협상 및 shared 타입 정비
- **5개국어 지원**: `ko`, `en`, `ja`, `zh-CN`, `es-419` 추가 및 LangToggle 개선
- **다국어 SEO 강화**: `canonical`, `hreflang`, `sitemap`, 구조화 데이터, 메타데이터 현행화
- **운영/검증 체계 개선**: namespace slicing, i18n 정합성 검사 스크립트/CI 정비

## v2.23.0 – 배포 환경 정비 및 알림/트리거 안정화

운영 배포 신뢰성과 케이스 처리 파이프라인의 복구력을 높였습니다.

- **알림 복구력 강화**: 케이스 알림 자동 재시도 로직 추가
- **원자적 트리거 도입(P0-7)**: 조건 충족 시 과금/알림 처리의 일관성 보장
- **배포 파이프라인 정비**: `.env.*.local` 분리, env 주입 순서 정리, migrate/seed 실행 안정화
- **문서/리뷰 반영**: i18n 아키텍처 계획 문서 추가 및 PR 피드백 반영

## v2.22.0 – 브랜드 명칭 통합

서비스 명칭을 `빈방어때`에서 `빈방`으로 통합하고 표기 일관성을 맞췄습니다.

- **브랜딩 통합**: UI/문서/문구 전반의 서비스 표기를 `빈방` 기준으로 정리

## v2.21.0 – 접수/케이스 파이프라인(P0-1~3) 도입

운영 케이스 생성 전 구간을 구조화해 접수 무결성과 상태 전이 안정성을 강화했습니다.

- **접수 무결성 강화(P0-1, P0-1A)**: Webhook 원문 스냅샷 고정, Q1~Q7 의미키 스키마 검증
- **상태 머신 파이프라인(P0-2)**: 케이스 처리 상태 전이 로직 정식 도입
- **모호성 처리(P0-3)**: `NEEDS_CLARIFICATION` 큐 추가로 분기 처리 명확화
- **외부 검증 대응**: Google OAuth 검증 관련 정책 링크/메타데이터 보완

## v2.20.0 – 공개 정책 페이지 추가

외부 검증과 사용자 신뢰도 강화를 위해 정책 문서를 공개 라우트로 제공했습니다.

- **개인정보처리방침 페이지 추가**: 공개 라우트 및 랜딩/인증 링크 연결
- **이용약관 페이지 추가**: 정책 문구 가독성 개선 및 공개 접근성 강화
- **배포 안정성 보완**: worker Dockerfile 및 notifications import 정리

## v2.19.0 – 튜토리얼 API 예외 처리 강화

첫 방문 튜토리얼 기능의 안정성을 높이고, API 입력 예외 케이스를 보완했습니다.

- **튜토리얼 동선 보강**: 대시보드 온보딩 Dialog 플로우 구현 정리
- **API 하드닝**: `PATCH /api/user/tutorial` invalid JSON 처리 강화
