# MoonCatch Sprint 2 — apps/web 통합 & 베타 준비
...
- `**mooncatch_watches` 테이블 없음**: 기존 `Accommodation` 테이블에 Agoda API 컬럼 추가
...
| W4-D4-T1 | ✅ 완료 (Sprint 3 W5-D1에서 인프라 완성) | `docs/mooncatch/staging-deployment-checklist.md`, `docker/docker-compose.staging.yml`, `apps/web/.env.staging.example` |
| W4-D4-T2 | ✅ 완료                           | `apps/web/src/services/admin/awin.service.ts` (31일 분할 + 20 calls/min)                                                  |
| W4-D5-T1 | ✅ 완료                           | `docs/mooncatch/beta-onboarding-guide.md`                                                                              |
| W4-D5-T2 | ✅ 완료                           | `reports/daily.sql`                                                                                                    |


참고:

- `staging` 인프라는 Sprint 3 W5-D1에서 완성 (`docker/docker-compose.staging.yml`, `apps/web/.env.staging.example`).
...
| 항목     | Sprint 1 (mooncatch 독립) | Sprint 2 (web 통합)                              |
| ------ | ----------------------- | ---------------------------------------------- |
...
`**mooncatch_*` 테이블**: Sprint 2에서 신규 생성 없음. Sprint 1 데이터는 마이그레이션 또는 폐기.
...
| Day 19 | W4-D4-T1 | 스테이징 배포          | `apps/web` 스테이징 환경                    | end-to-end 알림 1건 + 클릭아웃 1건 성공                 | 4h  |
...
| 스테이징 e2e  | 알림 등록 → 폴링 → 알림 → 클릭아웃            | 1건 완전 성공                      |
...
| W4-D4 | 스테이징 배포 완료                     | end-to-end 1건 성공           |
...
- **apps/mooncatch 퇴역**: `apps/web`으로 완전 전환, mooncatch 컨테이너 중단
