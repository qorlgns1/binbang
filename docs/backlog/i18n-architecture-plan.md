...
- Public/App 모두 `/{lang}/...` 형태로 통일 (정책에 따라 Public만 적용도 가능)
- 지원 언어: `ko`, `en`

핵심 규칙:

- URL에 locale이 있으면 **그 값이 Source of Truth**다.
- URL에 locale이 없으면 cookie/header/user preference로 협상 후 **redirect**한다.
...
export function middleware(req: NextRequest): NextResponse {
  ...
  // 2) 협상(예시): cookie -> Accept-Language -> default
...
- **CI**: PR에서 key set 정합성(11.1)을 검사하고, typegen이 최신인지도 확인한다(예: “생성 결과가 달라지면 실패”).
...
- `pnpm i18n:check`
  ...
  - ICU 변수 파라미터 정합성 검사
- `pnpm i18n:ci`
  - `i18n:typegen` 실행 후 “생성 결과가 바뀌는지” 검사(= 생성이 최신인지)
  - `i18n:check` 실행
...
- URL에 locale이 없는 경우: cookie/header/default 순으로 결정
- 로그인 사용자: user profile의 `preferredLocale`이 협상에 포함되는지
...
- 특정 임계치 초과 시(예: N KB) 빌드/리뷰 단계에서 탐지할 수 있는 가드(스크립트/리포트)를 둔다(선택).
