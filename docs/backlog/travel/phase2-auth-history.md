...
- [ ] P2-3-T2: 트리거 포인트 정의 및 구현:
  - 대화 저장 버튼 클릭 시
  - 히스토리 사이드바 접근 시
  - 게스트 5대화 초과 시 (채팅 차단 + 로그인 유도)
  - 장소 북마크 시도 시 (추후)
- [ ] P2-3-T3: "나중에" 버튼으로 모달 닫기 (5대화 초과 시에도 표시, 문구는 "더 쓰려면 로그인이 필요해요" 등으로 구분)
...
- [ ] P2-7-T2: Rate limiter 구현 (Redis 기반)
...
- [ ] 게스트 사용 제한이 정상 동작 (5대화 초과 시 채팅 차단 + 로그인 유도)
...
- Rate limiting: Redis 사용 (이미 인프라 있음). 다중 인스턴스에서도 제한 일관 적용. 일일 리셋은 UTC 자정 기준
- 대화 제목: 채팅 LLM 호출 시 같은 응답에서 제목/요약 필드 포함 요청 (시스템 프롬프트 또는 구조화 출력). 실패 시 첫 사용자 메시지 slice(0, 100) fallback
- 유저 메시지: 요약/변형 없이 원문을 `TravelMessage.content`(role=user)에 저장
